...
--3.Показать товар, для которого существует менее 3 размеров в той же подкатегории товаров (Таблица Production.Product). [Name], ProductSubcategoryID.
	select COUNT(ProductModelID) AS kolvo, ProductModelID from Production.Product WHERE Size IS NOT NULL group by ProductModelID having COUNT(ProductModelID)<3
--4.Показать товары, цена которых равна минимальной (больше нуля) цене товара из той же подкатегории (Таблица Production.Product). Показать поля [Name], ListPrice и ProductSubcategoryID.
	select [name],ProductSubcategoryID,ListPrice from Production.Product WHERE Listprice in (select MIN(ListPrice) as minprice from Production.Product WHERE ListPrice>0  group by ProductSubcategoryID)
--5.Показать товары, цена которых меньше средней цены в любом стиле (Таблица Production.Product). Показать поля [Name], ListPrice и Style.
	select [name],style,ListPrice from Production.Product WHERE style is not null and Listprice < (select AVG(ListPrice) as AVGprice from Production.Product WHERE style is not null )
